...
Optimized for Woocommerce 2.0.14

Author: SunnysideSoft
	...
	function update_checkout() {
		...
		xhr = $.ajax({
			...
			success: 	function( response ) {
				...
				jQuery('form.checkout').attr('name','PGIOForm'); // change formname to conform to PayGate style
				$("#btn_paygate_purchase").prettyPhoto({
					default_width: 350,
					modal: true,
					...
					<div class="pp_overlay"></div>',
					   changepicturecallback: function() {
						// copy and remove template contents
						jQuery('#PGIOscreenWrapper').data('html', jQuery('#PGIOscreenWrapper').html());
						jQuery('#PGIOscreenWrapper').empty(); // empty template to avoid #id collisions
						
						doTransaction(document.PGIOForm);
						
						// reset style after prettyPhoto opened
						jQuery('.pp_content').removeAttr('style');
						jQuery('.pp_content').attr('style','height:auto;');
						window.location.hash = '';
					   },
					   callback: function(){
						jQuery('#PGIOscreenWrapper').html(jQuery('#PGIOscreenWrapper').data('html'));
				...
				jQuery('#place_order').on('click', function () {
					...
					// 페이게이트가 결제수단으로 선택된경우 fancybox창을 띄우고 그 안에 결제폼 로딩
					var payGateCheckbox = jQuery('#payment_method_paygatekorea').get(0);
					if( payGateCheckbox.checked == true) {
						jQuery('input[name=tid], input[name=replycode], input[name=replyMsg] ').val(''); //required: reset tid, replycode, replyMsg fields.
